...
;--------------------------------------------------------
		...
		VERS_MINOR = 3 ; minor version number
...
;------------------------------------
	PREV = 1; previous char in string.
store:
	pushw x 
	pushw y
	clr a
...
;---------------------------------------
; sous-routine utilisée par store 
; transforme les séquences d'échappement
; en leur caractère ASCII.
; input:
;   A  caractère qui suit le '\'
; output:
;   A  caractère substitué ou 0 si échec.
;---------------------------------------
convert_escape:
	cp a,#'a'
	jrne 1$
