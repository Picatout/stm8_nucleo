...
```
 ...
 Les vecteurs sont en fait des instructions **JPF** (jump far). Cette instruction occupe 4 octets. Le code **0x82** suivit d'une adresse de 24 bits. Les routines de gestion des interruptions de termine par une instruction **iret** alors que les routines normales se termine par l'instruction **ret**. le mot **int** n'est pas une directive mais une macro qui est remplacé par l'instruction machine **jpf** (Jump far). Cette macro prend comme argument une étiquette qui représente le point d'entrée du gestionnaire d'interruption. L'assembleur remplace donc cette macro par un code de 4 octets qui commence par **0x82** qui est le code machine pour l'instruction **jpf**. Suit les 3 octets de l'adresse de la routine. Lorqu'une interruption est déclenchée, après avoir sauvegarder tous les registres le gestionnaire d'interruption mais la valeur **0x8000+4*int_nbr** dans le registre **PC** de sorte que la prochaine instruction excécuté est le **jump far** correspondant à l'entrée **int_nbr** de cette table. **int_nbr** est le numéro de l'interruption en commençant à zéro pour le **RESET**. 
